...
APPBASE="build/macos-x64/Celestial.app"
...
build() {
    ...
    cp native/build-x64/src/Celestial $APPBASE/Contents/MacOS/
...
dmg() {
    ...
    mv Celestial\ *.dmg Celestial-x64.dmg

    # dump for CI
    hdiutil imageinfo Celestial-x64.dmg

    if ! hdiutil imageinfo Celestial-x64.dmg | grep -q "Format: UDBZ" ; then
        echo "Format of resulting dmg was not UDBZ, make sure your create-dmg has support for --format"
        exit 1
    fi

    if ! hdiutil imageinfo Celestial-x64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Celestial-x64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Celestial-x64.dmg
    fi
}
