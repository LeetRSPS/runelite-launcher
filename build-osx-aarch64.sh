...
APPBASE="build/macos-aarch64/Celestial.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/Celestial.jar
    ...
    cp native/build-aarch64/src/Celestial $APPBASE/Contents/MacOS/
    cp build/libs/Celestial.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Celestial
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Celestial
    popd

    otool -l $APPBASE/Contents/MacOS/Celestial
}
...
dmg() {
    ...
    mv Celestial\ *.dmg Celestial-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Celestial-aarch64.dmg

    if ! hdiutil imageinfo Celestial-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Celestial-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Celestial-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Celestial-aarch64.dmg
    fi
}
